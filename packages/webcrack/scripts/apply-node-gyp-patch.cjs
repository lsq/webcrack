...
const ivmVer = major >= 26 ? 'isolated-vm-7' : 'isolated-vm-6';

// 先找到 isolated-vm 的位置
const ivmPath = require.resolve(`${ivmVer}/package.json`);
...
if (major >= 26) {
    console.log("Patch for node v26 is not aviliable. Replacing ...")

    let content = fs.readFileSync(ivmPatchFile, 'utf-8');
    content = content.replaceAll('24.14.0', '26.0.0'); // 或 content.replace(/oldStr/g, 'newStr')
    fs.writeFileSync(ivmPatchFile, content, 'utf-8');
}
